Pro Currents
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;Programa que presenta la evolución de las corrientes
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  indirectTerm=0		;activate with '1'
...
;  if f eq 100 and nPlanets eq 4 then out=17
  ...
  if f eq 1 and nPlanets eq 2 and indirectTerm eq 1 then out=52
  dischargeImagesFolder='~/Dropbox/visualization_tools/images/Currents/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output)
  if out eq 52 then dischargeImagesFolder='~/Dropbox/visualization_tools/images/CurrentsIndirectTerm/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output)
...
;Coloca la velocidad radial en los arreglos
  ...
  vrad_=strcompress(dataFolder+'/gasvrad'+string(output)+'.dat'$
                    ,/remove_all)
  openr,1,vrad_
  readu,1,vrad
  close,1
...
;Coloca la velocidad azimutal en los arreglos
  ...
  vtheta_=strcompress(dataFolder+'/gasvtheta'+string(output)+$
                      '.dat',/remove_all)
  openr,1,vtheta_
  readu,1,vtheta
  close,1
...
;  window,2,xsize=600,ysize=600
  Set_plot,'z'
  Device,Set_resolution=[600,600],Set_Pixel_Depth=24
  contour,zrange_prev,x,y,/fill,charsize=1.5,$
          xrange=[-30,30],yrange=[-30,30],nlevels=300,/xstyle,/ystyle,xtitle='AU',ytitle='AU'
...
;Accretion of gas (minus sign)
  ...
  xyouts,440,550,'Acc = -1e-11',charsize=1.5,color=230,/device
  xyouts,440,530,'Acc = -3.3e-11',charsize=1.5,color=200,/device
  xyouts,440,510,'Acc = -6.6e-11',charsize=1.5,color=170,/device
  xyouts,440,490,'Acc = -1e-10',charsize=1.5,color=140,/device
  xyouts,440,470,'Acc = -3.3e-10',charsize=1.5,color=110,/device
  xyouts,440,450,'Acc = -6.6e-10',charsize=1.5,color=80,/device
  xyouts,440,430,'Acc = -1e-9',charsize=1.5,color=50,/device
...
;  window,3,xsize=600,ysize=600
  ...
  contour,alog10(dens*2./2.25*1.e7),x,y,/fill,charsize=1.5,$
          xrange=[-30,30],yrange=[-30,30],nlevels=300,$
  ...
  device,get_decompose=old_decomposed,decomposed=0
  loadct,39
  contour,alog10(dens*2./2.25*1.e7),x,y,levels=[-3.,-2.,-1,-0.602059,-0.301029,-0.124938,0.,1.,2.],/overplot,$
...
;  window,4,xsize=600,ysize=600
  ...
  Device,Set_resolution=[600,600],Set_Pixel_Depth=24
  !P.MULTI = [0,1,1,0,0]
  contour,tau,x,y,charsize=1.5,levels=[0.5,2.0],$;levels=[-0.30103,0.30103],$
;  contour,tau,x,y,charsize=1.5,/fill
;          c_color=[100,200],$
;          xrange=[-30,30],yrange=[-30,30],$
          xrange=[-20,-10],yrange=[-20,-10],$
          /xstyle,/ystyle,xtitle='AU',ytitle='AU',/fill ;,$
;min_value=-2,max_value=3
;contour,alog10(tau),x,y,/fill,levels=[-0.30103,0.47712],c_color=[100,150],/overplot
;contour,alog10(tau),x,y,/fill,levels=[0.47712,3],c_color=[50,200],/overplot
;loadct,1
;contour,alog10(tau),x,y,/fill,levels=[-0.30103],c_color=[100],/overplot
;loadct,0
;contour,alog10(tau),x,y,/fill,levels=[0.47712],c_color=60,/overplot;,$
  f=strtrim(f,2)
  nPlanets=strtrim(nPlanets,2)
  xyouts,150,530,'f='+f,charsize=1.5,charthick=2,color=0,/device
  xyouts,150,500,nPlanets+' planets',charsize=1.5,charthick=2,color=0,/device
  xyouts,150,100,strcompress(string(abs(output*1000.))+' years'),charsize=1.5,charthick=2,color=0,/device
  device,get_decompose=old_decomposed,decomposed=0
  loadct,39
;  xyouts,390,540,'       !4s!X < 0.5',charsize=1.5,charthick=2,color=70,/device ;ñ=!Z(00f1)os
;  xyouts,390,520,'0.5 < !4s!X < 2.0',charsize=1.5,charthick=2,color=160,/device
;  xyouts,390,500,'2.0 < !4s!X',charsize=1.5,charthick=2,color=240,/device
  device,get_decompose=old_decomposed,decomposed=0
  loadct,0
  oplot,xcero,ycero,psym=2
  oplot,xplanet,yplanet,psym=1
...
;nrad=256
  ...
  dataFolderStructures='~/Dropbox/visualization_tools/images/StructuresQuantification/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output)
  if out eq 52 then dataFolderStructures='~/Dropbox/visualization_tools/images/StructuresQuantificationIndirectTerm/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output)
  gasdens_temp=dblarr(nsec,nrad)
  gasdens_l_m=dblarr(nsec,nrad,3,20)
  mfin=dblarr(3)
output_string=strtrim(output,2)
  for l=0,2 do begin
;    if l eq 0 then begin
l_string=strtrim(l,2)
      spawn,'ls -l ~/Dropbox/visualization_tools/images/StructuresQuantification/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output_string)+'/gasdens_l='+string(l_string)+'_m=* | wc -l',mfinal
    if out eq 52 then spawn,'ls -l ~/Dropbox/visualization_tools/images/StructuresQuantificationIndirectTerm/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output_string)+'/gasdens_l='+string(l_string)+'_m=* | wc -l',mfinal
    for j=1,30 do begin
       a=STRTRIM(j, 2)
       if (a eq mfinal) then mfinal=j
    endfor
    for m=1,mfinal do begin
...
;  contour,gasdens_l_m(*,*,0,0),xx,radii,/fill,/xstyle,/ystyle,$
  contour,9.6*dens*2./2.25*1.e7,xx,radii,/xstyle,/ystyle,/fill,levels=[min(9.6*dens*2./2.25*1.e7)],$;/fill,$
          xrange=[0,2.*!PI],ytitle='AU',xtitle='!N!4u!3!N',c_color=[160],$
          charsize=2,/ylog,yrange=[1.,100.],thick=2,color=0,BACKGROUND=255
;  xyouts,100,100,strcompress('t = '+string(abs(output*1000.))+'
;  a!Z(00f1)os'),charsize=1.5,color=0,/device
   ...
   for l=0,2 do begin
    if l eq 0 then begin 
      ...
      for m=1,mfin[l] do begin
       contour,9.6*gasdens_l_m(*,*,l,m)*2./2.25*1.e7,xx,radii,/cell_fill,levels=[min(9.6*dens*2./2.25*1.e7)],c_color=[70],/overplot
      endfor
    endif
    if l eq 1 then begin 
       ...
       loadct,9
       for m=1,mfin[l] do begin
         contour,9.6*gasdens_l_m(*,*,l,m)*2./2.25*1.e7,xx,radii,/cell_fill,levels=[0.5],c_color=[160],/overplot
       endfor
    endif
       if l eq 2 then begin 
         ...
         loadct,39
         for m=1,mfin[l] do begin
           contour,9.6*gasdens_l_m(*,*,l,m)*2./2.25*1.e7,xx,radii,/cell_fill,levels=[2.],c_color=[250],/overplot
         endfor
       endif
    endfor
...
;  contour,alog10(gasdens_l_m(*,*,2,1)*2./2.25*1.e7),xx,radii,/cell_fill,c_color=[250],/overplot  
  ...
  if out eq 52 then xyouts,200,520,'indirect term',charsize=1.5,color=255,/device
  ...
  Set_plot,'z'
  Device,Set_resolution=[1000,600],Set_Pixel_Depth=24
  contour,alog10(dens*2./2.25*1.e7),xx,radii,nlevels=300,/fill,/irregular,/xstyle,/ystyle,$
          xrange=[0,2.*!PI],/ylog,ytitle='AU',xtitle='!N!4u!3!N',$
          charsize=2,yrange=[1.,100.],min_value=alog10(min(dens*2./2.25*1.e7)),max_value=alog10(max(dens*2./2.25*1.e7)),color=0,BACKGROUND=255;-3,2.4
  oplot,HillplanetTheta,HillplanetR,color=255,psym=3
  colscale=dindgen(101)-50
