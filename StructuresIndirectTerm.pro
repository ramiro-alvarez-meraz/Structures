...
;Read the correct out
  if f eq 1 and nPlanets eq 2 then out=52

  for output=300,output_,50 do begin

  dischargeImagesFolder='~/Dropbox/visualization_tools/images/StructuresQuantificationIndirectTerm/f'+string(f)+'_p'+string(nPlanets)+'/output'+string(output)
...
;Optically thick:
  ...
  MeanFillingFactorAtGap(0)=Atau(0)/total(ATau(*))
  MeanFillingFactorAtGap(1)=Atau(1)/total(ATau(*))
  MeanFillingFactorAtGap(2)=Atau(2)/total(ATau(*))
