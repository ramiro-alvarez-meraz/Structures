  ...
  MeanSemiMajorAxisHigherResolution=dblarr(outputs+1)

  for i=0,nPlanets-1 do begin
     planetAsString = STRTRIM(i, 2)
     planet_=strcompress('~/ram_vogto/fargo/out51/planet'+planetAsString+'.dat',/remove_all)
     openr,1,planet_
     s=dblarr(9)
...
;        window,2,xsize=600,ysize=600
	...
	xyouts,430,460,'(448,384)',charsize=1.5,color=0,/device
        for k=1,outputs do begin
           SemiMajorAxis[k,i]=a[k]
        endfor
     endif
  ...
  for k=0,outputs do begin
     MeanSemiMajorAxisHigherResolution[k]=total(SemiMajorAxis(k,*))/nPlanets
  endfor
;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;
  if nPlanets eq 2 then begin
     ...
     print,'lalala'
     if f eq 1 then print,MeanSemiMajorAxisHigherResolution[outputs]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisHigherResolution[outputs]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisHigherResolution[outputs]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 3 then begin
     MeanInitSemiMajorAxis=(20.+12.5+7.5)/3. 
     if f eq 1 then print,MeanSemiMajorAxisHigherResolution[350]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisHigherResolution[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisHigherResolution[1000]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 4 then begin 

     MeanInitSemiMajorAxis=(20.+12.5+7.5+5.)/4.
     if f eq 1 then begin 
       print,MeanSemiMajorAxisHigherResolution[200],MeanSemiMajorAxisHigherResolution[350],MeanSemiMajorAxisHigherResolution[350]/MeanInitSemiMajorAxis
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
       HeightScale=0.029*MeanSemiMajorAxisHigherResolution[350]^(0.25)*(AU)
       Visc=0.01*Cs*HeightScale*ViscUnits
       ViscTime=(16.*MeanSemiMajorAxisHigherResolution[350]^2)/(12.*Visc)
       print,MeanSemiMajorAxisHigherResolution[350]*AU,Visc,ViscTime
     endif
     if f eq 10 then print,MeanSemiMajorAxisHigherResolution[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisHigherResolution[1000]/MeanInitSemiMajorAxis
  endif
