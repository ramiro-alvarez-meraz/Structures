...
;Used arrays:
  ...
  InnerPlanetRadii=dblarr(outputs+1)
  SemimajorAxis_InnerPlanet=dblarr(outputs+1)
...
;     SemiejeMin=min(SemiMajorAxis)
	...
	yplanet[i]=Rplanet[i]*sin(ThetaPlanet[i])
        Hillplanet[i]=aa[i]*(1.-e)*((1./3)*mplanet[i])^(1./3.)
        for j=0,nsec-1 do begin
...
;---------------------------------------------------------------------
	...
	if i eq 0 then InnerPlanetHill[n]=Hillplanet[i]   
        if i eq 0 then InnerPlanetRadii[n]=Rplanet[i]   
        if i eq 0 then SemimajorAxis_InnerPlanet[n]=aa[i]
;---------------------------------------------------------------------
print,total(AtauHill(i,*,n)),!PI*Hillplanet[i]*Hillplanet[i]
     endfor     
