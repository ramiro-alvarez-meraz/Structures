  ...
  MeanSemiMajorAxisIndirectTerm=dblarr(outputs+1)

  for i=0,nPlanets-1 do begin
     planetAsString = STRTRIM(i, 2)
     planet_=strcompress('~/ram_vogto/fargo/out52/planet'+planetAsString+'.dat',/remove_all)
     openr,1,planet_
     s=dblarr(9)
...
;        xyouts,430,520,'Planet 1',charsize=1.5,color=0,/device
        ...
        xyouts,200,490,'indirect term',charsize=1.5,color=0,/device
        for k=1,outputs do begin
...
;       xyouts,430,490,'Planet 2',charsize=1.5,color=60,/device
  ...
  for k=0,outputs do begin
     MeanSemiMajorAxisIndirectTerm[k]=total(SemiMajorAxis(k,*))/nPlanets
  endfor
;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;
  if nPlanets eq 2 then begin
     ...
     print,'lalala'
     if f eq 1 then print,MeanSemiMajorAxisIndirectTerm[outputs]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisIndirectTerm[outputs]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisIndirectTerm[outputs]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 3 then begin
     MeanInitSemiMajorAxis=(20.+12.5+7.5)/3. 
     if f eq 1 then print,MeanSemiMajorAxisIndirectTerm[350]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisIndirectTerm[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisIndirectTerm[1000]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 4 then begin 

     MeanInitSemiMajorAxis=(20.+12.5+7.5+5.)/4.
     if f eq 1 then begin 
       print,MeanSemiMajorAxisIndirectTerm[200],MeanSemiMajorAxisIndirectTerm[350],MeanSemiMajorAxisIndirectTerm[350]/MeanInitSemiMajorAxis
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
       HeightScale=0.029*MeanSemiMajorAxisIndirectTerm[350]^(0.25)*(AU)
       Visc=0.01*Cs*HeightScale*ViscUnits
       ViscTime=(16.*MeanSemiMajorAxisIndirectTerm[350]^2)/(12.*Visc)
       print,MeanSemiMajorAxisIndirectTerm[350]*AU,Visc,ViscTime
     endif
     if f eq 10 then print,MeanSemiMajorAxisIndirectTerm[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisIndirectTerm[1000]/MeanInitSemiMajorAxis
  endif
