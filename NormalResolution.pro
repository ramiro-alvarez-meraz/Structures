  ...
  MeanSemiMajorAxisNormalResolution=dblarr(outputs+1)
...
;        window,2,xsize=600,ysize=600
	...
	xyouts,430,460,'(384,256)',charsize=1.5,color=0,/device
        for k=1,outputs do begin
           SemiMajorAxis[k,i]=a[k]
        endfor
     endif
  ...
  for k=0,outputs do begin
     MeanSemiMajorAxisNormalResolution[k]=total(SemiMajorAxis(k,*))/nPlanets
  endfor
;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;
  if nPlanets eq 2 then begin
     ...
     print,'lalala'
     if f eq 1 then print,MeanSemiMajorAxisNormalResolution[outputs]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisNormalResolution[outputs]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisNormalResolution[outputs]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 3 then begin
     MeanInitSemiMajorAxis=(20.+12.5+7.5)/3. 
     if f eq 1 then print,MeanSemiMajorAxisNormalResolution[350]/MeanInitSemiMajorAxis
     if f eq 10 then print,MeanSemiMajorAxisNormalResolution[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisNormalResolution[1000]/MeanInitSemiMajorAxis
  endif
  if nPlanets eq 4 then begin 

     MeanInitSemiMajorAxis=(20.+12.5+7.5+5.)/4.
     if f eq 1 then begin 
       print,MeanSemiMajorAxisNormalResolution[200],MeanSemiMajorAxisNormalResolution[350],MeanSemiMajorAxisNormalResolution[350]/MeanInitSemiMajorAxis
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
       HeightScale=0.029*MeanSemiMajorAxisNormalResolution[350]^(0.25)*(AU)
       Visc=0.01*Cs*HeightScale*ViscUnits
       ViscTime=(16.*MeanSemiMajorAxisNormalResolution[350]^2)/(12.*Visc)
       print,MeanSemiMajorAxisNormalResolution[350]*AU,Visc,ViscTime
     endif
     if f eq 10 then print,MeanSemiMajorAxisNormalResolution[450]/MeanInitSemiMajorAxis
     if f eq 100 then print,MeanSemiMajorAxisNormalResolution[1000]/MeanInitSemiMajorAxis
  endif
